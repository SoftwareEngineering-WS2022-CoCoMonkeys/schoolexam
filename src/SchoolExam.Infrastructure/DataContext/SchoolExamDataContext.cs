...
public class SchoolExamDataContext : DataContextBase<SchoolExamDbContext>, ISchoolExamDataContext
{
    ...
    public IQueryable<Exam> Exams => Context.Exams.Include(x => x.GradingTable).Include(x => x.TaskPdfFile)
        .Include(x => x.Tasks).Include(x => x.Booklets).ThenInclude(x => x.PdfFile).Include(x => x.Booklets)
        .ThenInclude(x => x.PdfFile).Include(x => x.Booklets).ThenInclude(x => x.Pages);
    ...
    public IQueryable<SubmissionPage> SubmissionPages => Context.SubmissionPages.Include(x => x.PdfFile);
